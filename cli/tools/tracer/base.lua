...
function _ExecuteMatchers(compilerName, compilerPath, compilerArguments)
    ...
    for _, id in ipairs(languageIDs) do
        ...
        if matchers then
            for _, matcher in ipairs(matchers) do
                ...
                if match then
                    if match.replace and compilerReplacement == nil then
                        if #match.invocations ~= 1 then
                            Log(1,
                                'Matches with replace = true must have exactly one invocation.')
                            Log(1,
                                'Warning: Cancelling invocations for language id %s and a successful match against %s.',
                                id, compilerName)
                            break
                        end
                        -- The first match with `replace = true` gets to overwrite the compiler
                        compilerReplacement = match.invocations[1]
                    elseif match.replace and compilerReplacement ~= nil then
    ...
    if #invocations > 0 and compilerReplacement ~= nil then
        Log(1,
